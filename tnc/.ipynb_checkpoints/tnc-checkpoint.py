...
#from tnc.evaluations import ClassificationPerformanceExperiment
...
def epoch_run(loader, disc_model, encoder, device, w=0, optimizer=None, train=True, transfer=False):
    if train:
        if transfer:
            encoder.eval()
        else:
...
def learn_encoder(x, encoder, window_size, w, lr=0.001, decay=0.005, mc_sample_size=20,
                  n_epochs=100, path='simulation', device='cpu', augmentation=1, n_cross_val=1, cont=False, transfer=False):
    accuracies, losses = [], []
    for cv in range(n_cross_val):
        ...
        if transfer:
            checkpoint = torch.load('./ckpt/%s/checkpoint_%d.pth.tar'%('har', cv))
            encoder.load_state_dict(checkpoint['encoder_state_dict'])
        ...
        for epoch in range(n_epochs+1):
            ...
            epoch_loss, epoch_acc = epoch_run(train_loader, disc_model, encoder, optimizer=optimizer,
                                              w=w, train=True, device=device, transfer=transfer)
        ...
        plt.title("Loss")
        plt.legend()
        plt.savefig(os.path.join("./plots/%s"%path, "loss_%s.pdf"%transfer))
        plt.figure()
        plt.plot(np.arange(n_epochs+1), train_acc, label="Train")
        plt.plot(np.arange(n_epochs+1), test_acc, label="Test")
        plt.title("Accuracy")
        plt.legend()
        plt.savefig(os.path.join("./plots/%s"%path, "accuracy_%s.pdf"%transfer))
...
def main(is_train, data_type, cv, w, cont, transfer):
    ...
    if data_type == 'waveform':
        ...
        if is_train:
            with open(os.path.join(path, 'x_train.pkl'), 'rb') as f:
                x = pickle.load(f)
                x = x.float()
            learn_encoder(x, encoder, w=w, lr=1e-3, decay=1e-5, n_epochs=N_EPOCHS, window_size=window_size,
                          path='waveform', mc_sample_size=20, device=device, augmentation=5, n_cross_val=cv, transfer=transfer)
            ...
            track_encoding(x_test[0,:,:], y_test[0], encoder, window_size, 'waveform')
            for cv_ind in range(cv):
                plot_distribution(x_test, y_test, encoder, window_size=window_size, path='waveform', device=device,
                                  augment=100, cv=cv_ind, title='TNC')
                exp = ClassificationPerformanceExperiment(n_states=8, encoding_size=10, path='waveform', hidden_size=100,
                                                          in_channel=3, window_size=4, cv=cv_ind)
                # Run cross validation for classification
                for lr in [0.001, 0.01, 0.1]:
                    print('===> lr: ', lr)
                    tnc_acc, tnc_auc, e2e_acc, e2e_auc = exp.run(data='waveform', n_epochs=N_EPOCHS, lr_e2e=lr, lr_cls=lr)
                    print('TNC acc: %.2f \t TNC auc: %.2f \t E2E acc: %.2f \t E2E auc: %.2f'%(tnc_acc, tnc_auc, e2e_acc, e2e_auc))

    if data_type == 'har':
        window_size = 4
        path = 'data/athena/HAR'
        encoder = RnnEncoder(hidden_size=100, in_channel=3, encoding_size=10, device=device)

        if is_train:
            with open(os.path.join(path, 'x_train.pkl'), 'rb') as f:
                x = pickle.load(f)
            learn_encoder(torch.Tensor(x), encoder, w=w, lr=1e-3, decay=1e-5, n_epochs=N_EPOCHS, window_size=window_size,
            ...
            for cv_ind in range(cv):
                ...
                for lr in [0.001, 0.01, 0.1]:
                    print('===> lr: ', lr)
                    tnc_acc, tnc_auc, e2e_acc, e2e_auc = exp.run(data='har', n_epochs=N_EPOCHS, lr_e2e=lr, lr_cls=lr)
                    print('TNC acc: %.2f \t TNC auc: %.2f \t E2E acc: %.2f \t E2E auc: %.2f'%(tnc_acc, tnc_auc, e2e_acc, e2e_auc))
...
if __name__ == '__main__':
    ...
    main(args.train, args.data, args.cv, args.w, args.cont, args.transfer)
